...
def get_vrops_alerts(vrops_url, username, password, filename):
    ...
    for symptom in symptoms:
        ...
        if 'condition' in symptom['state']:
            if symptom['state']['condition']['type'] == "CONDITION_METRIC_EVENT":
                array.append(symptom['state']['condition']['type'])
                array.append(symptom['state']['condition']['hardThresholdEventType'])
                wr.writerow(array)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-F', '--filename', default='vrops-symptoms-condition-metric-event', help='filename')
