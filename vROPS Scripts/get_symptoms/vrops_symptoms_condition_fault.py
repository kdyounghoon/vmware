...
def get_vrops_alerts(vrops_url, username, password, filename):
    ...
    for symptom in symptoms:
        ...
        if 'condition' in symptom['state']:
            if symptom['state']['condition']['type'] == "CONDITION_FAULT":
                array.append(symptom['state']['condition']['type'])
                array.append(symptom['state']['condition']['faultKey'])
                array.append(symptom['state']['condition']['faultEvents'])
                wr.writerow(array)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-F', '--filename', default='vrops-symptoms-condition-fault', help='filename')
