...
for n_ip_addr_link, n_ip_addr in n_ip_addrs['documents'].items():
    ...
    try:
        n_vra.delete('/provisioning/uerp' + n_ip_addr_link).raise_for_status()
    except Exception as e:
        print('! could not delete ip address : {}'.format(str(e)))
        try:
            pp(n_vra.get('/provisioning/uerp' + n_ip_addr_link).json())
        except:
            pass
    else:
        count += 1
...
for n_range_link in n_ranges['documentLinks']:
    try:
        n_vra.delete('/provisioning/uerp' + n_range_link).raise_for_status()
    except Exception as e:
        print('! could not delete range : {}'.format(str(e)))
        try:
            pp(n_vra.get('/provisioning/uerp' + n_range_link).json())
        except:
            pass
    else:
        count += 1
...
for o_range in o_ranges.values():
    ...
    for n_subnet in n_subnets.values():
        if o_subnet['id'] == n_subnet['id'] and o_subnet['name'] == n_subnet['name']:
            ...
            try:
                n_vra.post('/iaas/api/network-ip-ranges', n_range).raise_for_status();
            except Exception as e:
...
o_ip_addrs = o_vra.get(
    "/provisioning/uerp/resources/ip-addresses?expand&$top=10000&$filter=(ipAddressStatus eq 'ALLOCATED')")
o_ip_addrs.raise_for_status()
o_ip_addrs = o_ip_addrs.json()
...
for o_ip_addr in o_ip_addrs.values():
    ...
    o_subnet = o_subnets[o_range['subnetLink']]
    try:
        o_intf_id = o_intfs[o_ip_addr['connectedResourceLink']]['id']
    except Exception as e:
        print('! {} could not find network interface on old, ip will be allocated on dummy interface : {}'.format(
            ip_address, str(e)))
        n_intf_link = 'dummy'
    else:
        n_intfs = n_vra.get(
            "/provisioning/uerp/resources/network-interfaces?expand&$filter=(id eq '{}')".format(o_intf_id)).json()
        if n_intfs['documentCount'] == 1:
            ...
            print('! {} network interface ids are duplicated on new, ip will be allocated on dummy interface'.format(
                ip_address))
            pp(n_intfs['documentLinks'])
            n_intf_link = 'dummy'
        else:
            print('! {} could not find network interfaces on new, ip will be allocated on dummy interface'.format(
                ip_address))
            n_intf_link = 'dummy'
    for n_subnet in n_subnets.values():
        if o_subnet['id'] == n_subnet['id'] and o_subnet['name'] == n_subnet['name']:
            for n_range_link in n_subnet['rangeLinks']:
                ...
                if o_range['name'] == n_range['name']:
                    ...
                    try:
                        n_ip_addr = n_vra.post('/provisioning/uerp/resources/ip-addresses', n_ip_addr).json()
                    except Exception as e:
                        print(
                            '! {} could not create ip address allocation, it might will be done at next subnet range : {}'.format(
                                ip_address, str(e)))
                        pp(n_ip_addr)
                    else:
                        if n_intf_link == 'dummy':
                            dummy_count += 1
                        else:
                            alloc_count += 1
                            ...
                            try:
                                n_vra.patch('/provisioning/uerp' + n_intf_link, n_intf)
                            except:
                                pass
                        total_count += 1
                        print('[(A{}-D{}-E{}) {} / {}] {} create ip address allocation'.format(alloc_count, dummy_count,
                                                                                               error_count, total_count,
                                                                                               count, ip_address))
                        break
            else:
                error_count += 1
                print('[(A{}-D{}-E{}) {} / {}] {} could not create ip address allocation by matching ranges'.format(
                    alloc_count, dummy_count, error_count, total_count, count, ip_address))
            break
    else:
        error_count += 1
        print(
            '[(A{}-D{}-E{}) {} / {}] {} could not create ip address allocation by matching subnets'.format(alloc_count,
                                                                                                           dummy_count,
                                                                                                           error_count,
                                                                                                           total_count,
                                                                                                           count,
